...
const processOrder = async ({ params }: { params: { id: number } }) => {
  ...
  const orderTotal = orderTotalUser.filter(
    (order) =>
      order.attributes.user.data.id === orderData.attributes.user.data.id
  );
  const ordersToProcessUser = orderTotal.filter(
    (order) =>
      order.attributes.paid === true && order.attributes.processed === false
  );
  const ordersToProcessUserMonth = ordersToProcessUser.filter(
    (order) =>
      order.attributes.month === orderData.attributes.month &&
      order.attributes.year === orderData.attributes.year
  );
